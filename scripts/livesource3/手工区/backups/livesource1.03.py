...
# 手工区文件路径
MANUAL_GAT = "scripts/livesource/手工区/港·澳·台.txt"
...
def generate_output_files(data_containers, dictionaries):
    ...
    # 合并所有地方台到一个分类
    print("🔗 合并所有地方台频道到单一分类...")
    all_local_channels = []
    url_set = set()
    
    # 收集所有地方台频道
    local_data_lists = [
        data_containers['beijing_lines'], data_containers['shanghai_lines'], 
        data_containers['tianjin_lines'], data_containers['chongqing_lines'],
        data_containers['guangdong_lines'], data_containers['jiangsu_lines'],
        data_containers['zhejiang_lines'], data_containers['shandong_lines'],
        data_containers['henan_lines'], data_containers['sichuan_lines'],
        data_containers['hebei_lines'], data_containers['hunan_lines'],
        data_containers['hubei_lines'], data_containers['anhui_lines'],
        data_containers['fujian_lines'], data_containers['shanxi1_lines'],
        data_containers['liaoning_lines'], data_containers['jiangxi_lines'],
        data_containers['heilongjiang_lines'], data_containers['jilin_lines'],
        data_containers['shanxi2_lines'], data_containers['guangxi_lines'],
        data_containers['yunnan_lines'], data_containers['guizhou_lines'],
        data_containers['gansu_lines'], data_containers['neimenggu_lines'],
        data_containers['xinjiang_lines'], data_containers['hainan_lines'],
        data_containers['ningxia_lines'], data_containers['qinghai_lines'],
        data_containers['xizang_lines']
    ]
    
    # 合并并去重
    for local_list in local_data_lists:
        for channel in local_list:
            if ',' in channel:
                try:
                    name, url = channel.split(',', 1)
                    if url not in url_set:
                        url_set.add(url)
                        all_local_channels.append(channel)
                except:
                    continue
    
    # 按频道名称排序
    all_local_channels_sorted = sorted(all_local_channels, key=lambda x: x.split(',')[0])
    
    print(f"✅ 地方台合并完成: {len(all_local_channels_sorted)} 个频道")
    
    # 添加合并的地方台分类
    all_lines_custom.extend(["🏠地方台,#genre#"] + all_local_channels_sorted + ['\n'])
