...
# 其他分类
others_lines = []
others_urls_set = set()  # 改为Set存储URL哈希 - 性能优化
aktv_lines = [] # AKTV频道
...
def check_url_existence(data_list, url):
    ...
    try:
        ...
        if isinstance(data_list, list):
            # 对于列表，我们提取URL部分进行查找
            urls_set = {item.split(',')[1] for item in data_list if ',' in item}
            return url not in urls_set
        else:
            # 如果是Set，直接查找
            return url not in data_list
    except Exception as e:
...
def process_channel_line(line):
    ...
    try:
        if "#genre#" not in line and "#EXTINF:" not in line and "," in line and "://" in line:
            ...
            # 其他频道分发 - 使用Set去重（性能优化）
            if not channel_added and url_hash not in others_urls_set:
                others_urls_set.add(url_hash)  # 使用Set的add方法
